...
for ($i=0;$i<sizeof($reg);$i++)
{
	...
	fwrite($txt,"|");
		...
		else if ($reg[$i]["Gestacion"] == '1' && ($reg[$i]["FechaTomaElisaVIHInput"] == '0000-00-80' || $reg[$i]["FechaTomaElisaVIHInput"] == '1845-01-01'))
		{
			fwrite($txt,'1800-01-01');
		}
	...
	fwrite($txt,"|");
		if ($reg[$i]["Sexo"] == 'M' || ($edad < 10 || $edad >= 60) || $reg[$i]["Gestacion"] == '2')
		{
			fwrite($txt,'0');
		}
		else if ($reg[$i]["Gestacion"] == '1' && ($reg[$i]["ResultadoElisaVIH"] == '0' || $YearFechaTomaElisaVIH > 1900))
		{
			fwrite($txt,'22');
		}
