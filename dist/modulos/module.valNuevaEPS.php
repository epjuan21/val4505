...
$año = substr($FechaFinal, 0, 4);
$mes = substr($FechaFinal, 5, 2);
$dia = substr($FechaFinal, 8, 2);

$nombreArchivo = $reg["0"]["CodigoHabilitacionIPS"]."_".$dia."".$mes."".$año."".".TXT";
...
for ($i=0;$i<sizeof($reg);$i++)
{
	...
	fwrite($txt,"|");
		if ($edad >= 3 && ($reg[$i]["HipotiroidismoCongenito"] == '2' || $reg[$i]["HipotiroidismoCongenito"] == '21') )
		{
			fwrite($txt,'0');
		}
		else
	...
	fwrite($txt,"|");
		if ($edadDias < 30 && $reg[$i]["ControlRecienNacidoInput"]) 
		{
			fwrite($txt,'1800-01-01');
		}
		else
	...
	fwrite($txt,"|");
		if ($edad < 45 || ($edad == 81 || $edad == 51 || $edad == 64 || $edad = 46)) 
		{
			fwrite($txt,'1845-01-01'); 
		} 
	...
	fwrite($txt,"|");
		...
		else if ($reg[$i]["FechaTomaElisaVIHInput"] == '1800-01-01')
		{
			fwrite($txt,'22');
		}
		else
	...
	fwrite($txt,"|");
		if ($reg[$i]["Sexo"] == 'M' || $reg[$i]["FechaCitologiaCUInput"] == '1845-01-01')
		{
			...
			{
				if ($reg[$i]["FechaCitologiaCUInput"] == '1800-01-01' || $reg[$i]["FechaCitologiaCUInput"] == '1835-01-01' || ($edad > 10 && $reg[$i]["FechaCitologiaCUInput"] == '1845-01-01' && $reg[$i]["Sexo"] == 'F'))
				{
					fwrite($txt,'0');
				}
				else
	...
	fwrite($txt,"|");
		if ($reg[$i]["Sexo"] == 'M' || $edadDias < 12775) //12775 Equivale a 35 Años
		{
			fwrite($txt,'0');
		}
		else if ($reg[$i]["Sexo"] == 'F'  && $edadDias >= 12775 && $reg[$i]["CodigoHabilitacionTomaMamografia"] == '0')
		{
			fwrite($txt,'999');
		}
		else
...
require_once("module.VaciarDirectorio.php");
?>